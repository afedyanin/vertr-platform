...
- работающая машинка без стратегий и автоматических сигналов - на ручном приводе
- архитектура в модульном монолите - без выделенных хостов - один хост на все приложение
- работающий хост с набором контроллеров: гейтвей, портфолио, ордера, маркет дата
- к стрим сервисам подключены хендлеры, в которых логируются сообщения и сохраняются в БД
- ручные операции (постинг ордеров) - проходят по флоу и сохраняются в БД
