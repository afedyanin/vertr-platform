...
- реализовать оверрайды по позициям - с учетом субакаунтов?
- проверить, что происходит в связки OrderTrades ->  TradeOperations
- запустить и прогнать тест по портфелю - открытие позиции, реверс позиции (сейчас не работает)
...
- [ ] реализовать работу с акаунтом - при внесенни суммы - отражать в операциях
...
- при старте приложения запрашивать активные акаунты в MDS и восстанавливать по ним портфели через Replay
- при получении события запрашивать портфель (если нет - создавать) и аплаить событие в портфель (extension method)
- реализовать Redis репозиторий для хранения портфеля и позиций (Get, Set)
- продумать схему ключей для портфеля и событий
- реализовать PUB для событий по изменению поотфелей\позиций
- иметь возможность очистки событий по трейдам и ордерам (по книге, акаунту)
- иметь возможность сгенерить историю изменения портфеля (набор снепшотов)

4. Реализовать сценарий валидации расчитанного поотфеля
